...
//		}
	...
	public int findArrayLength()
	{
		...
		{
			if(input.charAt(j) == '+'|| input.charAt(j) == '*' || input.charAt(j) == '/' || (input.charAt(j) == '-' && input.charAt(j+1) == '('))
			{
				length++;
			}
	...
	public String[] copyArray()
	{
		...
		while(j < input.length())
		{
			...
			if(input.charAt(j) == '+'|| input.charAt(j) == '*' || input.charAt(j) == '/' )//|| input.charAt(j) == '('|| input.charAt(j) == ')' )
			{
				i++;
				temp[i] = "";
				...
				j++;
				i++;
				temp[i] = ""; //This part is necessary because you can't concatenate if the value at the specified index is null.
